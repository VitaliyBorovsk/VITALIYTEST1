...
def text_render(value, font_size=45):
...
class Game:
    def __init__(self):
        ...
        self.running = True
        self.button = Button("Tap", 50, 450)
        self.food_button = Button("Food", 300, 450)
        self.rect_pos = (100, 100, 300, 200)
        self.is_open = False
        self.close_button = Button("X", 370, 110, 30, 30)

    def draw(self):
        self.screen.fill(WHITE)
        self.button.draw(self.screen)
        self.food_button.draw(self.screen)
        if self.is_open:
            pg.draw.rect(self.screen, GRAY, self.rect_pos)
        ...
        while self.running:
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    ...
                    if event.button == 1:
                        if self.button.is_hovered(event.pos):
                            pass
                        if self.food_button.is_hovered(event.pos):
                            self.is_open = True
                        if self.is_open and self.close_button.is_hovered(event.pos):
                            self.is_open = False
